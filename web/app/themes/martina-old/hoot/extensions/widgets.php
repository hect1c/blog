...
abstract class Hoot_WP_Widget extends WP_Widget {
	...
	function render_field( $field, $value, $repeater = array() ){
		...
		switch( $type ) {
			...
			case 'icon':
				...
				<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-icon-picked' ?>" class="hoot-of-icon-picked"><i class="fa <?php echo esc_attr( $value ) ?>"></i><span><?php _e( 'Select Icon', 'hoot' ) ?></span></div>
				<div id="<?php echo $this->hoot_get_field_id( $id, $repeater ) . '-icon-picker-box' ?>" class="hoot-of-icon-picker-box">
					<div class="hoot-of-icon-picker-list"><i class="fa fa-ban hoot-of-icon-none" data-value="0" data-category=""><span><?php _e( 'Remove Icon', 'hoot' ) ?></span></i></div><?php
			...
			case 'group':
				...
				?><div class="hoot-widget-field-group" data-id="<?php echo esc_attr( $id ) ?>">
					...
					?>
					<div id="add-<?php echo rand(1000, 9999); ?>" class="hoot-widget-field-group-add" data-iterator="<?php echo is_array( $value ) ? max( array_keys( $value ) ) : 0; ?>"><?php _e('Add', 'hoot') ?></div>
				</div>
				<?php
				break;
	...
	static function get_pages( $number, $post_type = 'page' ){
		...
		// $options_pages[''] = __( 'Select a page:', 'hoot' );
		$options_pages = array();
		$number = intval( $number );
