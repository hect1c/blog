...
class Hoot_Options_Media_Uploader {
	...
	static function hootoptions_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		$output .= '<input id="' . esc_attr( $id ) . '" class="upload' . $class . '" type="text" name="' . esc_attr( $name ) . '" value="' . $value . '" placeholder="' . __('No file chosen', 'hoot') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . esc_attr( $id ) . '" class="upload-button button" type="button" value="' . __( 'Upload', 'hoot' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . esc_attr( $id ) . '" class="remove-file button" type="button" value="' . __( 'Remove', 'hoot' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'Upgrade your version of WordPress for full media support.', 'hoot' ) . '</i></p>';
		}
	...
	static function enqueue_admin_options_media_scripts() {
		...
		wp_localize_script( 'hoot-options-media-uploader', 'hootoptions_l10n', array(
			'upload' => __( 'Upload', 'hoot' ),
			'remove' => __( 'Remove', 'hoot' )
		) );
	}
