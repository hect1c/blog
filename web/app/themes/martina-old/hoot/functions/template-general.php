...
function hoot_get_wp_link() {
	return sprintf( '<a class="wp-link" href="http://wordpress.org" title="%s">%s</a>', esc_attr__( 'State-of-the-art semantic personal publishing platform', 'hoot' ), __( 'WordPress', 'hoot' ) );
}
...
function hoot_get_wp_theme_link( $link = '', $anchor = '' ) {
	...
	$anchor = ( empty( $anchor ) ) ? sprintf( __( '%s WordPress Theme', 'hoot' ), $name ) : $anchor;

	$title = sprintf( __( '%s WordPress Theme', 'hoot' ), $name );
...
function hoot_get_loop_description() {
	...
	elseif ( is_search() )
		$loop_desc = sprintf( __( 'You are browsing the search results for &#8220;%s&#8221;', 'hoot' ), get_search_query() );

	elseif ( is_post_type_archive() )
		$loop_desc = get_post_type_object( get_query_var( 'post_type' ) )->description;

	elseif ( is_time() )
		$loop_desc = __( 'You are browsing the site archives by time.', 'hoot' );

	elseif ( is_day() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'hoot' ), hoot_single_day_title( '', false ) );

	elseif ( is_month() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'hoot' ), single_month_title( ' ', false ) );

	elseif ( is_year() )
		$loop_desc = sprintf( __( 'You are browsing the site archives for %s.', 'hoot' ), hoot_single_year_title( '', false ) );

	elseif ( is_archive() )
		$loop_desc = __( 'You are browsing the site archives.', 'hoot' );
...
function hoot_single_year_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_date( _x( 'Y', 'yearly archives date format', 'hoot' ) );
...
function hoot_single_week_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Week %1$s of %2$s', 'hoot' ), get_the_time( _x( 'W', 'weekly archives date format', 'hoot' ) ), get_the_time( _x( 'Y', 'yearly archives date format', 'hoot' ) ) );
...
function hoot_single_day_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_date( _x( 'F j, Y', 'daily archives date format', 'hoot' ) );
...
function hoot_single_hour_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_time( _x( 'g a', 'hour archives time format', 'hoot' ) );
...
function hoot_single_minute_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Minute %s', 'hoot' ), get_the_time( _x( 'i', 'minute archives time format', 'hoot' ) ) );
...
function hoot_single_minute_hour_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'hoot' ) );
...
function hoot_search_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . sprintf( __( 'Search results for &#8220;%s&#8221;', 'hoot' ), get_search_query() );
...
function hoot_404_title( $prefix = '', $display = true ) {
	...
	$title = $prefix . __( '404 Not Found', 'hoot' );
...
function hoot_developer_data( $commented = true ) {
	if ( current_user_can('manage_options') ) {
		...
		echo get_num_queries() . ' ' . __( 'queries.', 'hoot' ) . ' ';
		timer_stop(1);
		echo ' ' . __( 'seconds.', 'hoot' ) . ' ' . ( memory_get_peak_usage(1) / 1024 ) / 1024 . 'MB';
		if ( true === $commented ) echo ' -->';
	}
}