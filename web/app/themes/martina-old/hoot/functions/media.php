...
function hoot_attachment_id3_keys( $fields, $attachment, $context ) {
	...
	if ( 'display' === $context ) {
		...
		$fields['filesize']		 = __( 'File Size', 'hoot' );
		$fields['mime_type']		= __( 'Mime Type', 'hoot' );
		$fields['length_formatted'] = __( 'Run Time',  'hoot' );
	}

	if ( hoot_attachment_is_audio( $attachment->ID ) ) {

		$fields['genre']		= __( 'Genre',	'hoot' );
		$fields['year']		 = __( 'Year',	 'hoot' );
		$fields['composer']	 = __( 'Composer', 'hoot' );
		$fields['track_number'] = __( 'Track',	'hoot' );

		if ( 'display' === $context )
			$fields['unsynchronised_lyric'] = __( 'Lyrics', 'hoot' );
	}
